...
export default function ShoppingListScreen() {
  ...
  const { shoppingList, toggleShoppingItem, removeFromShoppingList, clearShoppingList, addToShoppingList } = useApp();
  ...
  const totalSaving = shoppingList.reduce((sum, i) => sum + (i.bestPrice || 0) * i.quantity, 0);

  const handleAdd = () => {
    if (!newProduct.trim()) return;
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    addToShoppingList({ eanCode: "", productName: newProduct.trim(), quantity: 1, checked: false });
    setNewProduct("");
    setAddMode(false);
  };
  ...
  const handleClear = () => {
    Alert.alert("Limpar lista?", "Todos os itens serão removidos.", [
      ...
      { text: "Limpar", style: "destructive", onPress: () => { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning); clearShoppingList(); } },
    ]);
  };

  const renderItem = ({ item }: { item: ShoppingItem }) => (
    <Pressable
      style={[styles.item, { backgroundColor: C.surfaceElevated, borderColor: item.checked ? C.success : C.border, opacity: item.checked ? 0.6 : 1 }]}
      onPress={() => {
    ...
    >
      ...
      <View style={styles.itemInfo}>
        ...
        {item.eanCode ? (
          <Text style={[styles.itemEan, { color: C.textMuted }]}>{item.eanCode}</Text>
        ) : null}
        {item.bestPrice ? (
      ...
      <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); removeFromShoppingList(item.id); }} style={styles.removeBtn}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: C.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 12, backgroundColor: C.background }]}>
        ...
        <View style={styles.headerActions}>
          <Pressable
            ...
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setAddMode(!addMode); }}
          >
      ...
      {/* Add Item Input */}
      {addMode && (
        <View style={[styles.addRow, { backgroundColor: C.backgroundSecondary, marginHorizontal: 16, marginBottom: 8 }]}>
          <TextInput
            ...
            returnKeyType="done"
          />
          <Pressable style={[styles.addConfirmBtn, { backgroundColor: C.primary }]} onPress={handleAdd}>
      ...
      {/* Total */}
      {totalSaving > 0 && (
        <View style={[styles.totalRow, { backgroundColor: C.primary, marginHorizontal: 16, marginBottom: 12 }]}>
          <Text style={styles.totalLabel}>Melhor total estimado</Text>
          <Text style={styles.totalValue}>R$ {totalSaving.toFixed(2).replace(".", ",")}</Text>
        </View>
      )}
        ...
        ListEmptyComponent={() => (
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptySubtitle, { color: C.textMuted }]}>
              Adicione produtos ou escaneie códigos de barra
            </Text>
            <Pressable
...
const styles = StyleSheet.create({
  ...
  addRow: { flexDirection: "row", alignItems: "center", borderRadius: 12, paddingLeft: 14, paddingRight: 6, paddingVertical: 8, gap: 8 },
  addInput: { flex: 1, fontSize: 15, fontFamily: "Inter_400Regular" },
  addConfirmBtn: { width: 32, height: 32, borderRadius: 8, alignItems: "center", justifyContent: "center" },
  totalRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12 },
